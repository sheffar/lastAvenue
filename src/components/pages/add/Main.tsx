          ...
          <div className="flex flex-col lg:flex-row gap-5 justify-between lg:items-center">
            ...
            <div className="flex gap-5 lg:gap-3">
              {[1, 2, 3,].map((size, key: number) => (
                ...
                } key={key} className={`w-8 h-8 lg:h-10 lg:w-10 hover:border-gray-300 rounded-full duration-300 cursor-pointer border-2 border-gray-400 ${key === activeColor ? 'shadow-lg border-black' : 'border-transparent'} ${key === 0 && 'bg-red-500'}  ${key === 1 && 'bg-purple-500'}  ${key === 2 && 'bg-emerald-500'} `}>
